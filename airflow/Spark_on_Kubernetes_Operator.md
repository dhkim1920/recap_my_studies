...
기존의 `spark-submit` 방식은 일회성 명령에 의존하며 Kubernetes의 Declarative 관리 방식을 따르기 어렵다.
...
- **Restart Policy**: 작업 실패 시 재시작 전략을 설정 (`Never`, `OnFailure`, `Always`).
- **Mutating Admission Webhook**: (옵션) Pod가 생성될 때 설정을 주입, 예를 들어 ConfigMap 마운트, 볼륨 설정, 환경 변수 주입 등을 자동화
- **Dynamic Allocation**: 리소스 사용량을 최적화하기 위해 실행 중 Executor 수를 동적으로 조절
- **Monitoring**: Prometheus와 연동하여 메트릭을 수집하는 기능을 내장
