...
2. **수동 커밋**: 명시적으로 메서드를 호출하여 처리 완료된 Offset만 커밋할 수 있으며 안정적이지만 복잡도가 증가한다.  
...
- consumer group ID를 변경 (보통 이걸 많이함)
- KafkaConsumer의 seek() 메서드 사용하여 위치 지정 (offset 위치를 찾는건 어렵다.)
...
## Kafka Lag
Kafka lag는 **Consumer가 최신 메시지를 얼마나 따라잡지 못하고 뒤처져 있는가를 나타내는 지표** 
→ 즉, **처리되지 않은 메시지 수**
...
- lag는 0으로 보이지만 내부 처리 로직은 여전히 진행 중일 수 있음  
  → 정확한 커밋 타이밍이 중요
